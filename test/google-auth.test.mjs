...
test("accepts a valid Google ID token and returns the profile", async () => {
  ...
  jwk.kid = "test-kid";
  const r = await verifyGoogleIdToken(await sign(base(), kp.privateKey), CLIENT, {
    jwks: [jwk],
    now: NOW,
  });
...
test("rejects wrong audience, wrong issuer, and expired tokens", async () => {
  ...
  assert.equal(
    await v(
      await sign({ ...base(), aud: "someone-else.apps.googleusercontent.com" }, kp.privateKey),
    ),
    null,
  );
  assert.equal(
    await v(await sign({ ...base(), iss: "https://evil.example" }, kp.privateKey)),
    null,
  );
