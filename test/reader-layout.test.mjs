...
const mobileCss = await readFile(new URL("../src/styles/product-mobile.css", import.meta.url), "utf8");
...
test("EPUB reader mode also receives layout-specific styling", () => {
  for (const layout of ["folio", "vertical", "immersive"]) {
    assert.match(productCss, new RegExp(`reader\\[data-mode="epub"\\]\\[data-layout="${layout}"\\]`));
  }
  assert.match(readerSource, /layout=\{layout\}/);
  assert.match(readerSource, /applyEpubTheme\(renditionRef\.current, \{ font, size, lead, rtheme, layout \}\)/);
});

test("mobile vertical mode remains vertical instead of falling back to horizontal", () => {
  assert.match(mobileCss, /\.reader\[data-layout="vertical"\] \.rd-col\{/);
