...
test("every .sql in migrations/ is discovered by the runner (no silent skips)", () => {
  ...
  assert.equal(
    all.length,
    discovered.length,
    `un-discovered .sql files: ${all.filter((f) => !discovered.includes(f)).join(", ")}`,
  );
...
test("discovered migrations are sorted and include the Turnkey + on-chain set", () => {
  ...
  for (const f of [
    "0013_turnkey.sql",
    "0014_turnkey_chains.sql",
    "0015_turnkey_passkey.sql",
    "0016_onchain.sql",
  ]) {
...
test("findMigrationProblems flags a badly-named (skippable) file", () => {
  const problems = findMigrationProblems(
    new URL("./fixtures/bad-migrations", import.meta.url).pathname,
  );
  assert.equal(
    problems.some((p) => p.includes("SKIPPED")),
    true,
  );
});
...
test("findMigrationProblems flags a numbering gap", () => {
  const problems = findMigrationProblems(
    new URL("./fixtures/gap-migrations", import.meta.url).pathname,
  );
  assert.equal(
    problems.some((p) => p.includes("not contiguous")),
    true,
  );
});

test("parseMigrateArgs reads target / baseline / dry-run flags", () => {
  assert.deepEqual(parseMigrateArgs(["--remote"]), {
    local: false,
    remote: true,
    baseline: false,
    dryRun: false,
  ...
  assert.deepEqual(parseMigrateArgs(["--local", "--baseline"]), {
    local: true,
    remote: false,
    baseline: true,
    dryRun: false,
