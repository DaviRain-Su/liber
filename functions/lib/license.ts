...
export function validatePublishableLicense(raw?: string | null): { ok: true; license: PublishableLicense } | { ok: false; reason: string } {
  const signals = restrictiveSignals(raw);
  if (signals.length) return { ok: false, reason: `license contains restrictive signal: ${[...new Set(signals)].join(", ")}` };
