...
export function solTransferMessage(
  fromB58: string,
  toB58: string,
  lamports: bigint,
  blockhashB58: string,
  ...
  const ix = concat([
    ...
    data,
  ]); // programIdIndex=2, accounts=[from,to]
...
export function solParseForSigning(
  txB64: string,
  signerB58: string,
): { messageHex: string; sigOffset: number } {
  ...
  const numReq = bytes[p];
  p += 3; // message header (3 bytes)
  const accs = readShortvec(bytes, p);
  p += accs.len;
  let slot = 0;
  for (let i = 0; i < numReq; i++) {
    let match = true;
    for (let k = 0; k < 32; k++)
      if (bytes[p + i * 32 + k] !== signer[k]) {
        match = false;
        break;
      }
    if (match) {
