...
export function evmAddressFromSignature(
  digestHex: string,
  rHex: string,
  sHex: string,
  v: number,
...
export function verifyEd25519(msgHex: string, sigHex: string, pubkeyHex: string): boolean {
  try {
    return ed25519.verify(hexToBytes(sigHex), hexToBytes(msgHex), hexToBytes(pubkeyHex));
  } catch {
    return false;
  }
}
...
export function verifySecp256k1(
  ...
  try {
    return secp256k1.verify(hexToBytes(rHex + sHex), hexToBytes(digestHex), hexToBytes(pubkeyHex), {
      prehash: false,
    });
  } catch {
    return false;
