...
const stripZeros = (b: Uint8Array): Uint8Array => { let i = 0; while (i < b.length && b[i] === 0) i++; return b.slice(i); };
...
export interface EvmTx {
  nonce: string; gasPrice: string; gas: string; to: string; value: string; data: string; chainId: number;
}
...
export function evmSigningDigestHex(tx: EvmTx): string {
  const fields = [...txFields(tx), numToBytes(BigInt(tx.chainId)), new Uint8Array(0), new Uint8Array(0)];
...
export function evmSignedRawTx(tx: EvmTx, rHex: string, sHex: string, recid: number): string {
  ...
  const fields = [...txFields(tx), numToBytes(v), stripZeros(hexToBytes(rHex)), stripZeros(hexToBytes(sHex))];
