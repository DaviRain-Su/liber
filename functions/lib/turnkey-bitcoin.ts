...
const reverse = (b: Uint8Array): Uint8Array => {
  const a = b.slice();
  a.reverse();
  return a;
};
...
export function p2wpkhProgram(addr: string): Uint8Array {
  ...
  if (witver !== 0 || program.length !== 20)
    throw new Error("仅支持原生隔离见证地址（bc1q P2WPKH）");
  return Uint8Array.from(program);
}
// scriptPubKey for P2WPKH: OP_0 <20-byte program>.
export const p2wpkhScript = (program: Uint8Array): Uint8Array =>
  concat([Uint8Array.of(0x00, 0x14), program]);
...
export function bip143Sighashes(
  inputs: SighashInput[],
  outputs: TxOutput[],
  version = 2,
  locktime = 0,
  ...
  const hashPrevouts = hash256(
    concat(inputs.map((i) => concat([reverse(hexToBytes(i.txid)), u32le(i.vout)]))),
  );
  const hashSequence = hash256(concat(inputs.map((i) => u32le(i.sequence))));
  const hashOutputs = hash256(
    concat(outputs.map((o) => concat([u64le(o.value), varint(o.script.length), o.script]))),
  );
...
export function derLowS(rHex: string, sHex: string): Uint8Array {
  ...
  const body = concat([
    derInt(hexToBytes(rHex.padStart(64, "0"))),
    derInt(hexToBytes(s.toString(16).padStart(64, "0"))),
  ]);
...
export function buildSignedTx(
  ...
  const vin = concat([
    ...
    ...inputs.map((i) =>
      concat([reverse(hexToBytes(i.txid)), u32le(i.vout), Uint8Array.of(0x00), u32le(i.sequence)]),
    ),
  ]);
  const vout = concat([
  ...
  const witness = concat(
    witnesses.map((stack) =>
      concat([varint(stack.length), ...stack.map((it) => concat([varint(it.length), it]))]),
    ),
  );
  return bytesToHex(
    concat([u32le(version), Uint8Array.of(0x00, 0x01), vin, vout, witness, u32le(locktime)]),
  );
}
