...
reading.get("/:bookId", async (c) => {
  ...
  const [hls, notes, progress, heatRows] = await Promise.all([
    ...
    all(
      c.env.DB,
      `SELECT sid, text, color, up FROM notes WHERE user_id = ? AND book_id = ?`,
      uid,
      bid,
    ),
    first(
      c.env.DB,
      `SELECT chapter_n, percent FROM progress WHERE user_id = ? AND book_id = ?`,
      uid,
      bid,
    ),
    all(
      c.env.DB,
      `SELECT sid, COUNT(DISTINCT user_id) AS n FROM highlights WHERE book_id = ? GROUP BY sid ORDER BY n DESC LIMIT 5000`,
      bid,
    ),
  ]);
...
reading.put("/:bookId/highlight", async (c) => {
  ...
  if (color === null) {
    await run(
      c.env.DB,
      `DELETE FROM highlights WHERE user_id = ? AND book_id = ? AND sid = ?`,
      uid,
      bid,
      sid,
    );
  ...
  if (!["hl-user", "hl-yellow", "hl-green"].includes(color))
    return c.json({ error: "无效的划线颜色" }, 400);
  await run(
    c.env.DB,
