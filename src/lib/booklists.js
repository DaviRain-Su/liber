...
export async function deleteBooklist(listId) {
  ...
  if (a?.booklists?.remove && !isLocalId(listId)) {
    try {
      await a.booklists.remove(listId);
      emit();
      return;
    } catch {
      /* fall through */
...
export async function renameBooklist(listId, name, desc) {
  ...
  if (a?.booklists?.update && !isLocalId(listId)) {
    try {
      await a.booklists.update(listId, { name, desc });
      emit();
      return;
    } catch {
      /* fall through */
...
export async function addBookToBooklist(listId, bookId) {
  ...
  if (a?.booklists?.addItem && !isLocalId(listId)) {
    try {
      await a.booklists.addItem(listId, bookId);
      emit();
      return;
    } catch {
      /* fall through */
  ...
  if (l) {
    ...
    if (!l.books.includes(bookId)) {
      l.books.unshift(bookId);
      writeLocal(lists);
    }
  }
}
...
export async function removeBookFromBooklist(listId, bookId) {
  ...
  if (a?.booklists?.removeItem && !isLocalId(listId)) {
    try {
      await a.booklists.removeItem(listId, bookId);
      emit();
      return;
    } catch {
      /* fall through */
...
export async function getBooklist(listId) {
  ...
  if (a?.booklists?.get && !isLocalId(listId)) {
    try {
      const r = await a.booklists.get(listId);
      if (r?.booklist) return r.booklist;
    } catch {
      /* fall through */
    }
...
export async function forkBooklist(listId) {
  ...
  if (a?.booklists?.fork) {
    try {
      const r = await a.booklists.fork(listId);
      emit();
      return r?.id || null;
    } catch {
      /* ignore */
...
export async function toggleSaveBooklist(listId) {
  ...
  if (a?.booklists?.save) {
    try {
      const r = await a.booklists.save(listId);
      emit();
      return r?.saved;
    } catch {
      /* ignore */
