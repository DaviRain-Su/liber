...
const I = {
  ...
  echo: (
    ...
    >
      ...
      <path d="M9.9 10.7 6.4 7.2M14.1 10.9 17.4 8.2M13.7 14.1 16.8 16.6" />
    </svg>
  ),
  agent: (
    <svg
...
function AppBar({
  ...
  return (
    <div className="appbar">
      ...
      <nav className="nav">
        {links.map(([k, label]) => (
          <a key={k} className={active === k ? "on" : ""} onClick={() => onNav(k)}>
            {label}
          </a>
        ))}
      </nav>
  ...
  const tabs = [
    ...
    [
      ...
      >
        ...
        <path d="M3 20c0-3 2.5-5 5-5s5 2 5 5M14 20c0-2.2 1.2-3.6 3-3.8" />
      </svg>,
    ],
    [
      "news",
...
function profileTarget(input) {
  ...
  if ((window.PEOPLE && window.PEOPLE[name]) || (window.ME && name === window.ME.name))
    return { name };
  return null;
}
...
function isFollowing(name) {
  return readFollow().includes(name);
}
