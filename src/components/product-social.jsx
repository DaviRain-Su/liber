...
function threadFromFeed(f) {
  return {
    ...
    root: {
      userId: f.userId,
      u: f.u || "读者",
      color: f.color || "#3a4fb0",
      when: f.when || "刚刚",
      t: f.t || f.title || f.quote || "",
      up: f.up || 0,
    },
    replies: [],
  };
...
function Social({ onOpenBook, onOpenGroup, onContinue }) {
  ...
  let published = [];
  try {
    const p = JSON.parse(localStorage.getItem("liber.shared"));
    if (Array.isArray(p)) published = p;
  } catch {}
  ...
  useEffS(() => {
    ...
    window.liberApi.shares
      .list()
      .then((r) => {
        if (r && Array.isArray(r.shares)) setApiConvos(r.shares);
      })
      .catch(() => {});
    window.liberApi
      .feed()
      .then((r) => {
        if (r && Array.isArray(r.feed)) setApiFeed(r.feed);
      })
      .catch(() => {});
    window.liberApi.groups
      .list()
      .then((r) => {
        if (r && Array.isArray(r.groups)) setApiGroups(r.groups);
      })
      .catch(() => {});
  }, []);
  const convos = apiConvos || [
    ...published,
    ...(allowSeedFallback ? window.SHARED_CONVOS || [] : []),
  ];
  const [expanded, setExpanded] = useSs(null); // convo id
  const [forkTree, setForkTree] = useSs(null); // convo whose tree is open
  const [saved, setSaved] = useSs(() => {
    try {
      return JSON.parse(localStorage.getItem("liber.saved.convos")) || [];
    } catch {
      return [];
    }
  });
  const toggleSave = (id) => {
    setSaved((s) => {
      ...
      localStorage.setItem("liber.saved.convos", JSON.stringify(n));
      return n;
    });
  ...
  return (
    <div className="app-screen">
      <div className="soc">
        <div className="soc-wrap">
          <div className="soc-head">
            ...
            <p className="soc-sub">
              书页的边缘从不安静——看看别人在同一句旁写下的批注、被划得最多的句子，和精彩的 AI 对话。
            </p>
          </div>
            ...
            <div className="soc-main">
                ...
                (feed.length ? (
                  feed.map((f, i) => (
                    <FeedCard
                      key={i}
                      f={f}
                      onOpenThread={() => setThread(threadFromFeed(f))}
                      onOpenBook={onOpenBook}
                      onOpenGroup={
                        f.kind === "group" ? () => onOpenGroup(f.groupId || groups[0]?.id) : null
                      }
                    />
                  ))
                ) : (
              ...
              {tab === "convos" && (
                <div className="soc-convos">
                  <div className="soc-convos-intro">
                    读者们把和 AI 聊出的精彩片段公开在这里。浏览、收藏，或者——
                    <b>接着 ta 停下的地方继续问</b>。
                  </div>
                  {convos.length ? (
                    convos.map((c) => (
                      <div className="soc-convo-item" key={c.id}>
                        ...
                        />
                        {commentsFor === c.id && (
                          <CommentsPanel targetType="share" targetId={c.id} />
                        )}
                      </div>
                    ))
                  ) : (
                    ...
                    groups.map((g) => (
                      <div className="gl-card" key={g.id} onClick={() => onOpenGroup(g.id)}>
                        <div className="glc-top">
                          ...
                          <div>
                            <div className="glc-n">{g.name}</div>
                            <div className="glc-bk">共读《{g.bookT || g.book}》</div>
                          </div>
      ...
      {forkTree && (
        <ForkTreeModal
          convo={forkTree}
          onClose={() => setForkTree(null)}
          onFork={(node, path) => {
            setForkTree(null);
            onContinue(branchConvo(forkTree, path));
          }}
        />
      )}
    </div>
...
function EmptySocial({ text, compact }) {
  return (
    <div className="pf-empty" style={{ padding: compact ? "10px 0" : 28 }}>
      {text}
    </div>
  );
}
    ...
    <div className="fc-top">
      ...
      <div className="fc-who">
        ...
        <div className="fc-act">
          {f.kind === "anno" && (
            <>
              批注了 <b>{f.book}</b> · {f.chap}
            </>
          )}
          {f.kind === "highlight" && (
            <>
              划线了 <b>{f.book}</b> · {f.chap}
            </>
          )}
          {f.kind === "convo" && (
            <>
...
function ThreadOverlay({ thread, onClose }) {
  ...
  useEffS(() => {
    if (!window.liberApi || !thread.key) {
      setLoading(false);
      return;
    }
    let live = true;
    window.liberApi.thread
      .get(thread.key)
      .then((r) => {
        if (live && r && Array.isArray(r.replies)) setReplies(r.replies);
      })
      .catch(() => {})
      .finally(() => {
  ...
  const add = () => {
    ...
    setReplies((r) => [
      ...r,
      { u: "你", color: "#3a4fb0", when: "刚刚", t: text, up: 0, mine: true },
    ]);
    if (window.liberApi && thread.key)
      window.liberApi.thread.reply(thread.key, text).catch(() => {});
    setDraft("");
  };
      ...
      <div className="thread-modal">
        ...
        <div className="tm-body">
          <div className="tm-note root">
            ...
            <div className="nb2">
              <div className="nm">
                ...
                <span className="when">{root.when}</span>
              </div>
          ...
          <div className="tm-replies">
            {loading && (
              <div className="pf-empty" style={{ padding: 16 }}>
                加载讨论中…
              </div>
            )}
              ...
              <div className="tm-note" key={i}>
                ...
                <div className="nb2">
                  <div className="nm">
                    ...
                    {r.mine && " · 你"} <span className="when">{r.when}</span>
                  </div>
...
function CommentsPanel({ targetType, targetId }) {
  ...
  useEffS(() => {
    let live = true;
    if (window.liberApi)
      window.liberApi.comments
        .list(targetType, targetId)
        .then((r) => {
          if (live) setList((r && r.comments) || []);
        })
        .catch(() => {
          if (live) setList([]);
        });
  ...
  const add = () => {
    ...
    setList((l) => [
      ...(l || []),
      { id: "local" + Date.now(), u: "你", color: "#3a4fb0", t, up: 0, when: "刚刚", mine: true },
    ]);
  ...
  const upvote = (cmId) => {
    ...
    setList((l) =>
      (l || []).map((cm) => (cm.id === cmId ? { ...cm, up: (cm.up || 0) + (on ? -1 : 1) } : cm)),
    );
  ...
  return (
    <div className="convo-comments" style={wrap}>
      <div style={{ fontSize: 13, fontWeight: 600, opacity: 0.7, marginBottom: 6 }}>
        评论{list ? ` · ${list.length}` : ""}
      </div>
      <div style={{ maxHeight: 260, overflowY: "auto" }}>
        {list === null && (
          <div style={{ fontSize: 13, opacity: 0.55, fontStyle: "italic" }}>加载中…</div>
        )}
        {list && list.length === 0 && (
            ...
            <div style={row} key={cm.id}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, opacity: 0.6 }}>
                  ...
                  {cm.mine && " · 你"} <span style={{ marginLeft: 6 }}>{cm.when}</span>
                  {cm.walrus && (
                    <span title={cm.walrus} style={{ marginLeft: 6, color: "var(--accent)" }}>
                      · 已存证
                    </span>
                  )}
                </div>
                ...
                <div
                  style={{
                    ...
                    marginTop: 2,
                    cursor: "pointer",
                    color: upvoted.includes(cm.id) ? "var(--accent)" : "inherit",
                    opacity: 0.7,
                    display: "inline-flex",
                    alignItems: "center",
                    gap: 3,
