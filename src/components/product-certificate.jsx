...
function Certificate({ bookId, onBack, onOpenBook }) {
  ...
  const bookQ = useQuery({
    ...
    queryFn: () => window.liberApi.books.get(bookId),
    enabled: !!window.liberApi,
  });
  const proofQ = useQuery({
    queryKey: ["book", bookId, "proof"],
    queryFn: () => window.liberApi.books.proof(bookId),
    enabled: !!window.liberApi,
  });
      ...
      <div className="cert">
        <div className="cert-wrap">
          <div className="crumb" style={{ paddingTop: 24 }}>
            <a onClick={onBack}>{book.t}</a> <span>/</span>{" "}
            <span style={{ color: "var(--ink)" }}>存证证书</span>
          </div>
          ...
          <div className="cert-sheet">
            ...
            <div className="cert-top">
              <div>
                ...
                <div className="cert-by">
                  {book.a} · {book.sub} · {book.year}
                </div>
              </div>
              <div
                className={`cert-badge ${verifying ? "checking" : steps.length === checks.length ? "ok" : ""}`}
              >
            ...
            <div className="cert-sec">
              ...
              {net && net.configured && (
                ...
                >
                  实时探测 · Walrus {net.walrus === false ? "⚠ 暂不可达" : "✓ 网络可达"} · Arweave{" "}
                  {net.arweave === false ? "⚠ 暂不可达" : "✓ 网关可达"}
                  {net.sui != null && (
                    <>
                      {" "}
                      · Sui{" "}
                      {net.sui
                        ? `✓ 链上活跃${net.checkpoint ? ` · #${net.checkpoint}` : ""}`
                        : "⚠ 暂不可达"}
                    </>
                  )}
                </div>
              ...
              {copies.map((c, i) => (
                <div
                  className={`copy-row ${steps.includes(c.net === "Walrus" ? "walrus" : c.net === "Arweave" ? "arweave" : "") ? "verified" : ""}`}
                  key={i}
                >
                  <span className={`copy-dot ${c.status}`} />
                  <div className="copy-net">
                    <div className="cn">{c.net}</div>
                    <div className="cd">{c.node}</div>
                  </div>
                  <code className="copy-id">{c.id}</code>
                  <span className="copy-stat">
                    {verifying &&
                    !steps.includes(
                      c.net === "Walrus" ? "walrus" : c.net === "Arweave" ? "arweave" : "x",
                    )
                      ? "…"
                      : "可达"}
                  </span>
            ...
            <div className="cert-sec">
              ...
              <div className="verify-grid">
                {checks.map((c) => {
                  ...
                  return (
                    <div
                      className={`verify-item ${done ? "done" : ""} ${verifying && !done ? "pending" : ""}`}
                      key={c.k}
                    >
                      <span className="vi-check">{done ? "✓" : verifying ? "○" : "·"}</span>
                      <div>
                        <div className="vi-l">{c.label}</div>
                        <div className="vi-d">{done ? c.detail : "待校验"}</div>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="cert-foot">
              <div className="cf-meta">
                <div>
                  <span className="k">存证时间</span>
                  <span className="v">2025-03-12 09:41 UTC</span>
                </div>
                <div>
                  <span className="k">索引对象</span>
                  <span className="v">{book.index}</span>
                </div>
              </div>
