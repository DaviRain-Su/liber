...
function MembershipCard() {
  const planQ = useQuery({
    ...
    queryFn: () => window.liberApi.billing.plan(),
    enabled: !!window.liberApi?.billing?.plan,
  });
  ...
  const quota =
    usage && usage.limit !== null
      ? `本月剩余 ${usage.remaining} / ${usage.limit} 次`
      : "本月不限量";
...
function Profile({ userId, onOpenBook, onBack, authUser, onLogout, onProfileUpdated }) {
  ...
  const [editForm, setEditForm] = useSp({
    name: "",
    handle: "",
    bio: "",
    seal: "",
  ...
  useEpf(() => {
    ...
    const load = () => {
      window.liberApi.readers
        .following()
        .then((r) => {
          if (live && Array.isArray(r?.readers)) setRemoteFollowingList(r.readers);
        })
        .catch(() => {});
    };
    load();
    ...
    return () => {
      live = false;
      window.removeEventListener("liber-following", load);
    };
  }, [isMe]);
        ...
        <div className="pf">
          <div className="pf-wrap" style={{ paddingTop: 60 }}>
            ...
            <p className="muted" style={{ marginTop: 24, fontSize: 18 }}>
              {profileLoading ? "正在加载读者主页…" : "没有找到这位读者的主页。"}
            </p>
          </div>
        </div>
      </div>
  ...
  const realStats =
    isMe && summary?.stats
      ? {
          ...
          following: remoteFollowingList.length || person.stats?.following || 0,
        }
      : person.stats || {};
  ...
  const publicNotes =
    isMe && summary?.highlights
      ? summary.highlights
          .filter((h) => h.note)
          .map((h) => ({ q: h.t, t: h.note, book: h.book, chap: h.chap, when: h.when }))
      : isMe
        ? (window.SEED_HL || [])
            .filter((h) => h.note && catalogTitles.has(h.book))
            .map((h) => ({ q: h.t, t: h.note, book: h.book, chap: h.chap, when: h.when }))
        : person.publicNotes || [];
          ...
          setRemotePerson((p) =>
            p
              ? {
                  ...
                  stats: {
                    ...(p.stats || {}),
                    followers: r?.followerCount ?? p.stats?.followers ?? 0,
                  },
                }
              : p,
          );
  ...
  const followingCount = isMe
    ? remoteFollowingList.length || followSet.length
    : realStats.following || 0;
  const followerCount = isMe
    ? realStats.followers
    : person.id
      ? realStats.followers || 0
      : (realStats.followers || 0) + (following ? 1 : 0);
        ...
        <div className="pf-wrap">
          <div className="pf-cover">
            {isMe ? (
              ...
              <div
                className="pf-cover-art"
                style={{
                  background: `linear-gradient(120deg, ${person.color}, color-mix(in oklab, ${person.color} 30%, #2e3a7a))`,
                }}
              ></div>
            )}
            {!isMe && onBack && (
          ...
          <div className="pf-id">
            <div
              className="pf-ava"
              style={{ background: `linear-gradient(135deg, ${person.color}, #2e3a7a)` }}
            >
              {person.seal}
            </div>
            <div className="pf-name">
              <h1>
                {person.name}
                {isMe && <span className="pf-you">你</span>}
              </h1>
              <div className="handle">
                {person.handle} · {person.joined}
              </div>
              ...
              {isMe ? (
                ...
                <>
                  ...
                  >
                    {following ? (
                      <>
                        <span className="pf-check">✓</span>已关注
                      </>
                    ) : (
                      "+ 关注"
                    )}
                  ...
                  <button
                    ...
                    onClick={() =>
                      window.dispatchEvent(
                        new CustomEvent("liber-open-dm", {
                          detail: {
                            userId: person.userId || person.id || userId,
                            name: person.name,
                            color: person.color,
                            seal: person.seal,
          ...
          {tab === "wallet" && isMe && person.turnkeyWallets && (
            <WalletTab
              wallets={person.turnkeyWallets}
              passkeyEnrolled={!!person.turnkey_passkey_at}
              userId={person.id}
          ...
          {tab === "notes" && (
            <div className="pf-notes">
              ...
              {publicNotes.length === 0 && (
                <div className="pf-empty">{person.name} 还没有公开批注。</div>
              )}
            </div>
          )}
          {tab === "following" && isMe && (
...
function FollowRow({ p, following }) {
  ...
  const toggle = () => {
    if (p.id && window.liberApi?.readers?.follow) {
      window.liberApi.readers
        .follow(p.id)
        .finally(() => window.dispatchEvent(new Event("liber-following")));
    } else {
  ...
  return (
    <div className="pf-frow">
      ...
      <div className="pf-frow-main" {...clickable(() => window.openProfile(target))}>
        ...
        <div className="meta">
          {(p.stats?.followers || 0).toLocaleString()} 关注者 · 读完 {p.stats?.finished || 0} 本
        </div>
      </div>
      <button
        type="button"
