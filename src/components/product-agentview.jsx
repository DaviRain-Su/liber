...
const MCP_TOOLS = [
  ...
  {
    name: "liber.get_conversations",
    sig: "(book, sid?) → Convo[]",
    desc: "读者×AI 的公开对话与 fork 树",
  },
  { name: "liber.get_echoes", sig: "(sid) → Echo[]", desc: "跨书呼应（连接层）" },
  {
    name: "liber.get_graph",
    sig: "(limit?) → {nodes,edges}",
    desc: "全馆思维链接图谱（书↔书呼应网络）",
  },
...
function JLine({ k, v, t, last, indent = 1 }) {
  ...
  return (
    <div className="out" style={{ paddingLeft: indent * 16 }}>
      ...
      {k && ": "}
      <span className={cls}>{t === "str" ? `"${v}"` : String(v)}</span>
      {!last && ","}
    </div>
  );
}
...
function AgentView({ context, onClose, onCopy, onSquare, onGraph }) {
  ...
  const addr = book
    ? {
        ...
        blob: book.blob,
        index: book.index,
        license: book.license || "CC0-1.0",
      }
    : {
        uri: "liber://registry",
        blob: "walrus://0x00…root",
        index: "sui::registry::Library",
        license: "CC0-1.0",
      };

  const platformQ = useQuery({
    queryKey: ["platform", "status"],
    queryFn: () => window.liberApi.platform.status(),
    enabled: !!corpus && !!window.liberApi?.platform?.status,
  });
          ...
          <div className="av-body">
            <div className="av-sec">
              ...
              <div className="av-addr">
                <div className="ar">
                  ...
                  <code>
                    liber://charts/{ctxCharts.win}/{ctxCharts.metric}
                  </code>
                  <span
                    className="copy"
                ...
                <div className="ar">
                  ...
                  <code>every 10m · on-chain</code>
                </div>
                <div className="ar">
                  <span className="k">license</span>
                  <code>CC0-1.0 · 无需鉴权</code>
                </div>
              </div>
            </div>
            <div className="av-sec">
              ...
              <div className="term">
                ...
                <div className="tbody">
                  <div className="cmd">
                    <span className="pr">agent ❯</span> liber.get_charts(
                    <span className="str">"{winMap[ctxCharts.win]}"</span>,{" "}
                    <span className="str">"{ctxCharts.metric}"</span>)
                  </div>
                  <div className="out" style={{ margin: "6px 0 12px" }}>
                    → 读取链上聚合信号…
                  </div>
                  ...
                  {top.map((r, i) => {
                    ...
                    const val =
                      ctxCharts.metric === "surge" ? (surge[r.id] || 0) + "%" : r[ctxCharts.metric];
                    return (
                      <div className="out" key={r.id} style={{ paddingLeft: 16 }}>
                        {"{ "}
                        <span className="key">"rank"</span>: <span className="num">{i + 1}</span>,{" "}
                        <span className="key">"title"</span>: <span className="str">"{b.t}"</span>,{" "}
                        <span className="key">"{ctxCharts.metric}"</span>:{" "}
            ...
            <div className="av-sec">
              ...
              <div className="av-tools">
                <div className="av-tool">
                  <div className="at-top">
                    <code className="at-name">导读 Agent</code>
                  </div>
                  <div className="at-desc">追踪飙升榜，自动给冒头的书生成导读</div>
                </div>
                <div className="av-tool">
                  <div className="at-top">
                    <code className="at-name">推荐 Agent</code>
                  </div>
                  <div className="at-desc">把热句榜接入"接着读"，顺着潮流荐书</div>
                </div>
                <div className="av-tool">
                  <div className="at-top">
                    <code className="at-name">研究 Agent</code>
                  </div>
                  <div className="at-desc">按 30 天趋势做公共阅读的计量分析</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <>
      ...
      <div className="agentview-drawer">
        <div className="av-head">
          <div className="av-orb">{I.agent}</div>
          <div>
            <div className="av-t">Agent 视角</div>
            <div className="av-s">
        ...
        <div className="av-body">
          ...
          <div className="av-sec">
            ...
            <div className="av-addr">
              ...
              <div className="ar">
                ...
                <code>{addr.index}</code>
              </div>
              <div className="ar">
                <span className="k">license</span>
                <code>{licenseLabel(addr.license)}</code>
              </div>
            </div>
          </div>
            ...
            <div className="av-sec">
              ...
              <div className="av-platform-grid">
                {[
                  [
                    "Vectorize",
                    platform?.capabilities?.vectorize,
                    `${platform?.counts?.semanticDocs ?? "—"} docs`,
                  ],
                  [
                    "Queues",
                    platform?.capabilities?.queues,
                    `${platform?.counts?.pendingJobs ?? "—"} pending`,
                  ],
                  [
                    "D1 + R2",
                    platform?.capabilities?.d1 && platform?.capabilities?.r2,
                    `${platform?.counts?.jobs ?? "—"} jobs`,
                  ],
                  [
                    "AI Gateway",
                    platform?.capabilities?.aiGateway,
                    platform?.provider?.model || "Workers AI",
                  ],
                  ["Logs / Trace", platform?.capabilities?.logsAndTrace, "ops events"],
                  [
                    "Browser",
                    platform?.capabilities?.browserRendering,
                    `${platform?.counts?.shareAssets ?? "—"} assets`,
                  ],
                ].map(([name, on, meta]) => (
              ...
              <div className="tbody">
                ...
                {book ? (
                  <>
                    ...
                    <div className="out" style={{ paddingLeft: 16 }}>
                      <span className="key">"addressable"</span>: {"["}
                    </div>
                    <div className="out" style={{ paddingLeft: 32 }}>
                      <span className="str">"chapter"</span>,{" "}
                      <span className="str">"paragraph"</span>,{" "}
                      <span className="str">"sentence"</span>,{" "}
                      <span className="str">"highlight"</span>,{" "}
                      <span className="str">"conversation"</span>
                    </div>
                    <div className="out" style={{ paddingLeft: 16 }}>
                      {"],"}
                    </div>
            ...
            <div className="av-tools">
              {MCP_TOOLS.map((t) => (
                <div className="av-tool" key={t.name}>
                  <div className="at-top">
                    <code className="at-name">{t.name}</code>
                    <code className="at-sig">{t.sig}</code>
                  </div>
          ...
          <div className="av-sec">
            ...
            <div className="term">
              ...
              <div className="tbody">
                <div className="cmd">
                  <span className="pr">agent ❯</span> liber.{book ? "get_top_highlight" : "query"}(
                  <span className="str">"{book ? book.t : "最畅销的哲学公版书"}"</span>)
                </div>
                <div className="out" style={{ margin: "6px 0 12px" }}>
                  → 经 MCP 读取链上索引…
                </div>
                ...
                <div className="out" style={{ paddingLeft: 16 }}>
                  ...
                  <span className="str">"{book ? book.t : "道德经"}"</span>,
                </div>
                <div className="out" style={{ paddingLeft: 16 }}>
                  <span className="key">"top_highlight"</span>:{" "}
                  <span className="str">"知人者智，自知者明"</span>,
                </div>
                <div className="out" style={{ paddingLeft: 16 }}>
                  <span className="key">"liners"</span>:{" "}
                  <span className="str">{book ? book.liners : 8402}</span>,
                </div>
                <div className="out" style={{ paddingLeft: 16 }}>
                  <span className="key">"blob"</span>: <span className="str">{addr.blob}</span>
                </div>
                <div className="out">{"}"}</div>
                <div className="cmd" style={{ marginTop: 10 }}>
                  <span className="pr">agent ❯</span> <span style={{ opacity: 0.5 }}>▍</span>
