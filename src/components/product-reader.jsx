...
function TocList({ tocRows, readMode, activeEpubHref, chN, tx, sameEpubHref, jumpEpubTo, jumpTo }) {
  ...
  const v = useVirtualizer({
    ...
    getItemKey: (i) => {
      const t = tocRows[i];
      return (t && (t.key || t.href || t.n)) ?? i;
    }, // stable data key, not the index
  });
  ...
  const activeIndex = tocRows.findIndex((t) =>
    readMode === "epub" && t.href ? sameEpubHref(t.href, activeEpubHref) : t.n === chN,
  );
  useE(() => {
...
const profileRef = (x) =>
  x?.userId ? { userId: x.userId, name: x.u || x.name } : x?.u || x?.name || x;
...
function aiReply(mode, q, ctx) {
  ...
  if (mode === "translate")
    return {
      t: `今译：${ctx || q}。\n字词：逐句对照原文理解，先抓主语和动作，再看虚词转折。\n提醒：这是本地离线兜底；联网后会用 Workers AI 给出更准确的今译。`,
      ref: "古文今译 · offline",
    };
  if (mode === "debate")
    return {
      t: `那我反问你：你说的这个理解，换一个处境还成立吗？${ctx ? `比如把「${ctx}」放到完全相反的情形里——它会不会反而说明了相反的事？` : ""}先别急着下结论，我们一层层往下问。`,
      ref: "反问 · 苏格拉底式",
    };
  if (mode === "notes")
    return {
  ...
  if (mode === "extend")
    return {
      t: `${c}的这个概念，在馆里还能顺藤摸瓜：庄子《逍遥游》把它推得更远，亚当·斯密「看不见的手」也有几分影子。要不要对照读一段？`,
      ref: "↗ 跳转：庄子 · 逍遥游",
    };
  if (mode === "stoic")
    return {
      t: `换一个斯多葛的问法：${c}里，哪一部分在你的掌控之内，哪一部分不在？奥勒留会提醒你——困住人的从不是事本身，而是我们对它下的判断。`,
      ref: "斯多葛导师 · community/stoa",
    };
  if (mode === "textual")
    return {
      t: `先咬一下字：${c}的关键字，历代注本读法并不一致。王弼本与帛书本此处就有出入——把字源摊开，意思往往自己显形。`,
      ref: "考据派 · community/kaoju",
    };
  if (mode === "skeptic")
    return {
      t: `我先不接受这个说法。${c}——凭什么这样断定？它的反例是什么？换一个前提，结论还站得住吗？我们把它怀疑到底，剩下的才算数。`,
      ref: "怀疑论者 · community/pyrrho",
    };
  if (mode === "econ")
    return {
      t: `用激励和成本来看${c}：谁付出、谁得益？如果把它放进一个有稀缺和选择的处境里，人们的行为会怎么变？文本背后往往藏着一笔账。`,
      ref: "经济学家之眼 · community/smith",
    };
  return {
    t: `可以这样理解${c}：老子想说的，往往不在字面，而在字面留出的那道缝里。读慢一点，让句子自己展开——有需要我随时在旁边。`,
    ref: "道德经 · 第一章",
  };
}
...
function storedReaderMode(book) {
  try {
    return localStorage.getItem(readerModeKey(book?.id)) === "text"
      ? "text"
      : defaultReaderMode(book);
  } catch {
...
function applyEpubScriptMode(rendition, mode, originalText) {
  ...
  for (const content of contents)
    applyChineseScriptToDocument(epubContentDocument(content), mode, originalText);
}
      ...
      .then((mod) => {
        ...
        rendition.hooks?.content?.register?.((contents) => {
          applyChineseScriptToDocument(
            epubContentDocument(contents),
            scriptModeRef.current,
...
function Reader({ bookId, startChapter, onClose, continueConvo, onOpenBook }) {
  ...
  const seedBook =
    findCatalogBook(bookId) ||
    (catalogHasLiveBooks() ? missingBook : getCatalogBooks()[0]) ||
    missingBook;
  ...
  useE(() => {
    ...
    window.liberApi.books
      ...
      .then((res) => {
        ...
        setChapters((rows) =>
          rows.map((row) =>
            row.n === ch.n ? { ...next, placeholder: false, status: "ready" } : row,
          ),
        );
      })
      .catch(() => {
  ...
  const setReaderLayout = useCb((value) => {
    ...
    setLayout(next);
    try {
      localStorage.setItem("liber.reader.layout", next);
    } catch {
      /* ignore */
    }
  ...
  useE(() => {
    const h = (e) => {
      if (READER_LAYOUTS.has(e.detail)) setLayout(e.detail);
    };
  ...
  const load = (k, d) => {
    const v = localStorage.getItem("liber.set." + k);
    return v == null ? d : v;
  };
  ...
  useE(() => {
    localStorage.setItem("liber.set.font", font);
  }, [font]);
  useE(() => {
    localStorage.setItem("liber.set.size", size);
  }, [size]);
  useE(() => {
    localStorage.setItem("liber.set.lead", lead);
  }, [lead]);
  useE(() => {
    localStorage.setItem("liber.set.meas", meas);
    localStorage.setItem("liber.set.ver", "2");
  }, [meas]);
  useE(() => {
    localStorage.setItem("liber.set.rtheme", rtheme);
  }, [rtheme]);
  useE(() => {
    localStorage.setItem("liber.set.script", scriptMode);
  }, [scriptMode]);
  ...
  useE(() => {
    localStorage.setItem("liber.set.pagemode", pageMode);
  }, [pageMode]);
  ...
  const fontFam =
    {
      song: "var(--body)",
      kai: "'Cormorant Garamond', 'KaiTi', serif",
      hei: "'IBM Plex Mono', sans-serif",
    }[font] || "var(--body)";

  /* highlights + user notes (persisted per book) */
  const hlKey = "liber.hl." + book.id,
    ntKey = "liber.nt." + book.id;
  ...
  useE(() => {
    localStorage.setItem(hlKey, JSON.stringify(hls));
  }, [hls]);
  useE(() => {
    localStorage.setItem(ntKey, JSON.stringify(notes));
  }, [notes]);
  ...
  useE(() => {
    ...
    let cancelled = false;
    window.liberApi.reading
      .get(book.id)
      .then((r) => {
        if (cancelled || !r) return;
        if (r.highlights && Object.keys(r.highlights).length)
          setHls((prev) => ({ ...r.highlights, ...prev }));
        if (r.notes && Object.keys(r.notes).length)
          setNotes((prev) => {
            const merged = { ...prev };
  ...
  const [feed, setFeed] = useS(() =>
    continueConvo
      ? [
          {
            who: "bot",
            t: `你正接着 ${continueConvo.author?.name || "一位读者"} 关于《${continueConvo.bookT}》的这段对话往下问。下面是 ta 之前聊到的——继续就好。`,
            ref: "接续对话 · " + continueConvo.chap,
          },
          ...continueConvo.msgs.map((m) => ({
            who: m.r === "q" ? "user" : "bot",
            t: m.t,
            ref: m.r === "q" ? undefined : "原对话",
          })),
        ]
      : [
          {
            who: "bot",
            t: `我正在陪你读这本《${seedBook.t}》。读到哪、卡在哪，问我就好——我会尽量只基于当前文本回答。`,
            ref: "通读陪伴模式",
          },
        ],
  );
  ...
  useE(() => {
    ...
    if (typeof window !== "undefined" && window.liberApi)
      window.liberApi.reading
        .progress(book.id, ch.n, Math.round((cIdx / chapterCount) * 100))
        .catch(() => {});
  ...
  const applyHl = (color) => {
    ...
    if (window.liberApi)
      sids.forEach((id) => window.liberApi.reading.highlight(book.id, id, color).catch(() => {}));
  ...
  const openNotePop = (sid, e) => {
    ...
    if (window.liberApi && srvAnno[sid] === undefined)
      window.liberApi
        .annotations(book.id, sid)
        .then((res) => {
          if (res && Array.isArray(res.annotations))
            setSrvAnno((p) => ({ ...p, [sid]: res.annotations }));
        })
        .catch(() => {});
  };
  ...
  const addNote = (sid) => {
    ...
    setNotes((prev) => ({
      ...
      [sid]: [
        ...(prev[sid] || []),
        { u: "你", color: "#3a4fb0", t: text, up: 0, replies: 0, mine: true },
      ],
    }));
    ...
    if (typeof window !== "undefined" && window.liberApi) {
      window.liberApi.ai
        .chat({
          ...
          question: q,
          context: ctx,
          chapter: `第 ${ch.n} 章 · ${ch.title}`,
        })
        .then((res) => {
          if (res && res.text && !res.error)
            setFeed((f) => [...f, { who: "bot", t: res.text, ref: res.ref }]);
          else fallback();
        })
        .catch(fallback)
        .finally(finish);
  ...
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (pendingLast.current) {
        pendingLast.current = false;
  ...
  useE(() => {
    if (!pendingLast.current) setPage(0);
  }, [cIdx]);
  ...
  useE(() => {
    const onKey = (e) => {
      if (e.key === "Escape") {
        ...
        else if (setOpen) setSetOpen(false);
        else if (notePop) setNotePop(null);
        else onClose();
      }
      if (e.key === "ArrowRight" && !aiOpen) {
        paged ? advance(1) : go(1);
      }
      if (e.key === "ArrowLeft" && !aiOpen) {
        paged ? advance(-1) : go(-1);
      }
    };
  ...
  const chapterBody = (
    <>
      <div className="rd-chap-no">
        第 {String(ch.n).padStart(2, "0")} 章{ch.paras.length ? ` · 约 ${chapMins} 分钟` : ""}
      </div>
      <h1 className="rd-chap-title">{tx(ch.title)}</h1>
      <div className="rd-chap-rule" />
      {ch.truncated && (
        <div
          className="rd-chap-partial"
          role="status"
          style={{
            margin: "0 0 14px",
            padding: "10px 14px",
            borderRadius: 8,
            background: "rgba(176,90,32,0.10)",
            border: "1px solid rgba(176,90,32,0.30)",
            color: "#8a4a1f",
            fontSize: 13,
            lineHeight: 1.6,
          }}
      ...
      <div className="rd-text">{ch.paras.map(renderParagraph)}</div>
      {(() => {
            ...
            <div className="rce-cards">
              {nextChap && (
                <div className="rce-card" onClick={() => go(1)}>
                  ...
                  <div className="rce-mid">
                    <div className="rce-lab">本书 · 接着读</div>
                    <div className="rce-t">
                      第 {nextChap.n} 章 · {tx(nextChap.title)}
                    </div>
                  </div>
              ...
              {crossRec && (
                <div className="rce-card" onClick={() => onOpenBook && onOpenBook(crossRec.bookId)}>
                  ...
                  <div className="rce-mid">
                    <div className="rce-lab">跨书 · {tx(echo.theme)}</div>
                    <div className="rce-t">
                      {tx(crossRec.bookT)} · {tx(crossRec.chap)}
                    </div>
                    <div className="rce-why">{tx(crossRec.why)}</div>
                  </div>
                  <span className="rce-go">{I.right}</span>
                </div>
              )}
  ...
  const activeEpubItem =
    readMode === "epub" && activeEpubHref
      ? epubToc.find((item) => sameEpubHref(item.href, activeEpubHref))
      : null;
  const tocRows =
    readMode === "epub" && epubToc.length
      ? epubToc
      : toc.length
        ? toc
        : chapters.map((c) => ({ n: c.n, title: c.title, has: true }));
  const waitingForEpubToc = readMode === "epub" && hasEpub && !epubToc.length;
  const seekTextProgress = useCb(
    (e) => {
      const el = scrollRef.current;
      if (!el) return;
      const r = e.currentTarget.getBoundingClientRect();
    ...
    >
      ...
      <div className="rd-bar">
        ...
        <div className="rd-title">
          ...
          <span className="ch">
            {readMode === "epub"
              ? `EPUB${activeEpubItem?.title ? ` · ${tx(activeEpubItem.title)}` : ""}`
              : `第 ${ch.n} 章 · ${tx(ch.title)}`}
          </span>
        </div>
        ...
        <div className="rd-mode-toggle" role="group" aria-label="阅读模式">
          ...
          <button
            ...
            onClick={() => setReadMode("epub", { persist: false })}
          >
            {I.book} EPUB
          </button>
        </div>
        ...
        {readMode === "epub" ? (
          ...
          <div className="rd-scroll" ref={scrollRef}>
            <div className="rd-empty">
              <div
                className="rd-empty-seal"
                style={{ background: book.cls ? undefined : "var(--accent)" }}
              >
                {book.seal || "书"}
              </div>
              ...
              {hasEpub ? (
                ...
                <>
                  <p className="rd-empty-p">
                    Liber 只收录 CC0 / 公共领域文本，这本书的逐句精读版正在录入。
                  </p>
                  <p className="rd-empty-p rd-empty-sub">
                    可以先读《道德经》《论语》《孙子兵法》——它们已经有逐句正文，支持划线、批注和问
                    AI。
                  </p>
                  <div className="rd-empty-actions">
              ...
              {isCurl && flip && (
                <div className={"rd-curl " + (flip.dir > 0 ? "fwd" : "bwd")} aria-hidden="true">
                  <div className="rd-curl-face" style={{ background: flip.bg }}>
                    <div
                      className="rd-col rd-paged-flow"
                      style={{
                        columnWidth: colw + "px",
                        columnGap: gap + "px",
                        transform: `translateX(${-flip.show * step}px)`,
                      }}
                    >
                      {chapterBody}
                    </div>
      ...
      <div className="rd-foot">
        ...
        <div className="pg-track" onClick={seekTextProgress}>
          ...
          <div
            className="pg-dot"
            style={{ left: readMode === "epub" ? "0%" : barProg * 100 + "%" }}
          />
        </div>
        <div className="pg-label">
          {readMode === "epub" ? "EPUB 阅读" : `第 ${ch.n} 章 · 全书 ${pct}%`}
        </div>
      </div>
          ...
          return (
            <>
              ...
              <div
                className="note-pop"
                style={{
                  left: Math.min(Math.max(notePop.x, 180), window.innerWidth - 180),
                  top: notePop.y,
              ...
              >
                    ...
                    <div className="np-note" key={i}>
                      ...
                      <div>
                        ...
                        <div className="mt">
                          ...
                          <span>回复 {n.replies || 0}</span>
                          {n.mine && pinBtn(notePop.sid)}
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="np-add">
                  <input
                    autoFocus={notePop.focus}
                    placeholder="写下你的批注，其他读者会看见…"
                    value={noteDraft}
          ...
          <div className="toc-drawer">
            ...
            {waitingForEpubToc ? (
              ...
              <TocList
                tocRows={tocRows}
                readMode={readMode}
                activeEpubHref={activeEpubHref}
                chN={ch.n}
                tx={tx}
                sameEpubHref={sameEpubHref}
                jumpEpubTo={jumpEpubTo}
                jumpTo={jumpTo}
              />
            )}
          </div>
        ...
        <>
          ...
          <div className="ai-drawer">
            <div className="dh">
              ...
              <div>
                <div className="nm">
                  {(window.AGENTS[(window.LENSES.find((l) => l.id === aiMode) || {}).agent] || {})
                    .name || "书友"}
                </div>
                <div className="role">
      ...
      {lensPicker && (
        <LensPicker
          ...
          onSummon={(id) => {
            setSummoned((s) => (s.includes(id) ? s : [...s, id]));
            setAiMode(id);
            setLensPicker(false);
            setAiOpen(true);
          }}
      ...
      {toast && (
        <div className="rd-toast">
          ...
          <div>
            <div className="t-t">{toast}</div>
            <div className="t-s">已发布到共读页 · 别人可以接着这段继续问</div>
          </div>
        </div>
      )}
...
function ShareComposer({ book, ch, aiCtx, feed, continueConvo, onClose, onPublish }) {
  ...
  const draftConvo = {
    ...
    bookT: book.t,
    seal: book.seal,
    chap: `第 ${ch.n} 章 · ${ch.title}`,
    quote,
    title: msgs.find((m) => m.r === "q")?.t,
    insight,
  ...
  const publish = () => {
    ...
    let arr = [];
    try {
      arr = JSON.parse(localStorage.getItem("liber.shared")) || [];
    } catch {}
    arr.unshift(rec);
    localStorage.setItem("liber.shared", JSON.stringify(arr));
    if (window.liberApi)
      window.liberApi.shares
        .publish({
          bookId: book.id,
          form,
          title: draftConvo.title,
          insight,
          quote,
          visibility: vis,
          chap: draftConvo.chap,
          seal: book.seal,
          msgs,
          parentId,
        })
  ...
  const toggle = (idx) =>
    setPicked((p) =>
      p.includes(idx) ? p.filter((x) => x !== idx) : [...p, idx].sort((a, b) => a - b),
    );
          ...
          <div className="sh-controls">
            ...
            {form === "insight" && (
              <div className="sh-row">
                <div className="sh-lab">
                  提炼的金句 <span className="hint">AI 已草拟，可改</span>
                </div>
                <textarea
            ...
            <div className="sh-row">
              <div className="sh-lab">
                纳入哪几轮{" "}
                <span className="hint">
                  {picked.length} / {seed.length} 条
                </span>
              </div>
              <div className="sh-turns">
                {seed.map((t) => (
                  <label
                    className={`sh-turn ${picked.includes(t.idx) ? "on" : ""} ${t.r}`}
                    key={t.idx}
                  >
                    <input
                      type="checkbox"
                      checked={picked.includes(t.idx)}
    ...
    <>
      ...
      <div className="lens-modal">
          ...
          {official.map((l) => (
            ...
            >
              ...
              <div className="lens-mid">
                <div className="lens-nm">{l.name}</div>
                <div className="lens-pitch">{l.pitch}</div>
              </div>
              <span className="lens-pick">{active === l.id ? "使用中" : "切换"}</span>
            </div>
          ))}
          <div className="lens-grp">
            社区 · 可发布 <span className="lens-grp-x">任何人都能训练并发布一个视角</span>
          </div>
          {community.map((l) => {
            ...
            return (
              ...
              >
                ...
                <div className="lens-mid">
                  <div className="lens-nm">
                    {l.name}{" "}
                    <span className="lens-prov">
                      ✓ 已签名 · 被引用 {ag.cited?.toLocaleString?.() || ag.cited}
                    </span>
                  </div>
                  <div className="lens-pitch">{l.pitch}</div>
                  <div className="lens-by">
                    {ag.model} · {ag.by}
                  </div>
                </div>
                <span className="lens-pick">
                  {active === l.id ? "使用中" : on ? "再次使用" : "召唤"}
                </span>
              </div>
            );
          })}
        ...
        <div className="lens-foot">
          视角是开放生态的一部分 · 由社区训练、署名发布、可被任何 Agent 调用
        </div>
      </div>
    </>
  );
...
function AIPanel({
  ...
  const active = (window.LENSES || []).filter(
    (l) => l.tag === "official" || (summoned || []).includes(l.id),
  );
  return (
    <>
      ...
      <div className="ai-feed" ref={feedRef}>
        {feed.map((m, i) =>
          m.who === "user" ? (
            <div className="ai-bub user" key={i}>
              {m.t}
            </div>
          ) : (
            <div className="ai-bub bot" key={i}>
...
function RailAnnotations({ ch, annoFor, tx = (value) => value }) {
  ...
  return (
    <div className="rail-section">
      ...
      {sentences.map((s) =>
        annoFor(s.id).map((n, i) => (
          <div className="rail-note" key={s.id + i}>
            {i === 0 && (
              <div className="rn-q">
                「{tx(s.t).length > 22 ? tx(s.t).slice(0, 22) + "…" : tx(s.t)}」
              </div>
            )}
            <div className="rn-row">
              <div className={"ava" + (n.ai ? " agent" : "")} style={{ background: n.color }}>
                {n.ai ? "AI" : n.u[0]}
              </div>
              <div>
                <ProvBadge note={n} />
