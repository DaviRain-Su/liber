...
// Tiny dependency-free force layout: repulsion + spring edges + centering.
function layout(nodes, edges, W, H, iters = 280) {
  const cx = W / 2, cy = H / 2;
  ...
  const links = edges.map((e) => ({ s: byId.get(e.source), t: byId.get(e.target), w: e.weight })).filter((l) => l.s && l.t);
  for (let it = 0; it < iters; it++) {
    ...
    for (let i = 0; i < pts.length; i++) {
      for (let j = i + 1; j < pts.length; j++) {
        ...
        const rep = 5200 / d2; // Coulomb repulsion
        const d = Math.sqrt(d2);
        const fx = (dx / d) * rep, fy = (dy / d) * rep;
        a.vx += fx; a.vy += fy; b.vx -= fx; b.vy -= fy;
      }
    }
    for (const l of links) { // Hooke spring (stronger pairs sit closer)
      let dx = l.t.x - l.s.x, dy = l.t.y - l.s.y;
      const d = Math.sqrt(dx * dx + dy * dy) || 0.01;
      const rest = 150 - Math.min(l.w, 6) * 10;
      const f = (d - rest) * 0.02;
      const fx = (dx / d) * f, fy = (dy / d) * f;
      l.s.vx += fx; l.s.vy += fy; l.t.vx -= fx; l.t.vy -= fy;
    }
    for (const p of pts) {
      p.vx += (cx - p.x) * 0.012; p.vy += (cy - p.y) * 0.012; // gravity to center
...
function GraphView({ onClose, onOpenBook }) {
  ...
  const { pts, links } = React.useMemo(
    () => (state.nodes.length ? layout(state.nodes, state.edges, W, H) : { pts: [], links: [] }),
