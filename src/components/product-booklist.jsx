...
import {
  loadMyBooklists, createBooklist, deleteBooklist, renameBooklist,
  addBookToBooklist, removeBookFromBooklist, getBooklist, forkBooklist, toggleSaveBooklist,
  subscribeBooklists,
...
function resolveBook(catalog, id) {
  return catalog.find((b) => b.id === id) || findCatalogBook(id) ||
    { id, t: id, a: "", sub: "", cls: "ink", seal: "书" };
}
...
function AddToBooklist({ bookId, onClose }) {
  ...
  useE(() => { refresh(); const off = subscribeBooklists(refresh); return off; }, [bookId]);
    ...
    <div className="bl-pop-scrim" onClick={onClose}>
      <div className="bl-pop" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bl-pop-list">
          ...
          {lists && lists.map((l) => (
            <button key={l.id} className={"bl-pop-row" + (l.has ? " on" : "")} disabled={busy} onClick={() => toggle(l)}>
        ...
        {creating ? (
          <div className="bl-pop-create">
            <input autoFocus placeholder="新书单名称…" value={name} maxLength={40}
              onChange={(e) => setName(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") create(); }} />
            <button className="btn btn-primary" disabled={busy || !name.trim()} onClick={create}>创建</button>
          </div>
        ) : (
          <button className="btn btn-ghost btn-block" onClick={() => setCreating(true)}>＋ 新建书单</button>
        )}
      </div>
    </div>
  );
...
function BookPicker({ listId, existing, onClose }) {
  ...
  useE(() => { const off = subscribeCatalog((b) => setCatalog(b)); return off; }, []);
  const has = new Set(existing || []);
  const term = q.trim().toLowerCase();
  const list = catalog.filter((b) => {
    if (!term) return true;
    return [b.t, b.a, b.sub].some((s) => String(s || "").toLowerCase().includes(term));
  }).slice(0, 60);
  ...
  return (
    <div className="bl-pop-scrim" onClick={onClose}>
      <div className="bl-pop bl-pop-wide" onClick={(e) => e.stopPropagation()}>
        ...
        <input className="bl-search" autoFocus placeholder="搜书名、作者…" value={q} onChange={(e) => setQ(e.target.value)} />
        <div className="bl-pick-grid">
          {list.map((b) => (
            <button key={b.id} className={"bl-pick" + (has.has(b.id) ? " on" : "")}
              disabled={has.has(b.id)} onClick={() => addBookToBooklist(listId, b.id)}>
              <Cover book={b} />
...
function Booklist({ listId, onBack, onOpenBook }) {
  ...
  const refresh = () => getBooklist(listId).then((l) => { setList(l); setSaved(!!l?.saved); setLoading(false); });
  useE(() => { setLoading(true); refresh(); const off = subscribeBooklists(refresh); return off; }, [listId]);
  useE(() => { const off = subscribeCatalog((b) => setCatalog(b)); return off; }, []);

  const mine = !!list?.mine;
  const books = list ? (list.books || []).map((id) => resolveBook(catalog, id)) : [];

  const startEdit = () => { setNameDraft(list?.name || ""); setEditing(true); };
  ...
  const remove = async (bookId) => { await removeBookFromBooklist(listId, bookId); await refresh(); };
  const onDelete = async () => { await deleteBooklist(listId); onBack(); };
  const onFork = async () => { const id = await forkBooklist(listId); if (id) onBack(); };
  const onSave = async () => { const s = await toggleSaveBooklist(listId); if (s != null) setSaved(s); };
  const copyLink = () => {
    try {
  ...
  if (loading && !list) {
    return <div className="app-screen"><div className="bl-screen"><div className="bl-wrap"><div className="bl-empty">加载中…</div></div></div></div>;
  }
  if (!list) {
    return (
      <div className="app-screen"><div className="bl-screen"><div className="bl-wrap">
        <div className="crumb"><a onClick={onBack}>我的书架</a> <span>/</span> <span style={{ color: "var(--ink)" }}>书单</span></div>
  ...
  return (
    <div className="app-screen">
      <div className="bl-screen">
        <div className="bl-wrap">
            ...
            <div className="bl-head-mid">
              {editing ? (
                <div className="bl-edit">
                  <input autoFocus value={nameDraft} maxLength={40}
          ...
          <div className="bl-actions">
            {mine ? (
              <>
                <button className="btn btn-primary" onClick={() => setAdding(true)}>＋ 添加书籍</button>
                {!editing && <button className="btn btn-ghost" onClick={startEdit}>重命名</button>}
                <button className="btn btn-ghost" onClick={copyLink}>{copied ? "已复制链接 ✓" : "复制书单链接"}</button>
                <button className="btn btn-ghost bl-danger" onClick={onDelete}>删除书单</button>
              </>
            ) : (
              <>
                <button className="btn btn-primary" onClick={onFork}>Fork 到我的书单</button>
                <button className="btn btn-ghost" onClick={onSave}>{saved ? "已收藏 ✓" : "收藏书单"}</button>
                <button className="btn btn-ghost" onClick={copyLink}>{copied ? "已复制链接 ✓" : "复制书单链接"}</button>
              </>
            )}
          </div>
          ...
          {books.length === 0 ? (
            <div className="bl-empty">
              这个书单还没有书。{mine ? "点「添加书籍」从书库挑选，或在书籍详情页点「加入书单」。" : ""}
            </div>
          ) : (
            <div className="bl-books">
              {books.map((b) => (
                <div className="bl-book" key={b.id}>
                  ...
                  <div className="bl-book-t" onClick={() => onOpenBook(b.id)}>{b.t}</div>
                  <div className="bl-book-a">{b.a}</div>
                  {mine && <button className="bl-book-rm" title="移出书单" onClick={() => remove(b.id)}>{I.x}</button>}
                </div>
              ))}
            </div>
          )}
      ...
      {adding && <BookPicker listId={listId} existing={list.books || []} onClose={() => { setAdding(false); refresh(); }} />}
    </div>
  );
}
