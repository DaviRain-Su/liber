...
function Delta({ d }) {
  ...
  return (
    <span className={"ch-delta " + (d > 0 ? "up" : "down")}>
      {d > 0 ? "▲" : "▼"}
      {Math.abs(d)}
    </span>
  );
...
function Surge({ p }) {
  return (
    <span className={"ch-surge " + (p >= 0 ? "up" : "down")}>
      {p >= 0 ? "▲" : "▼"} {Math.abs(p)}%
    </span>
  );
}
...
function Charts({ onOpenBook, onBack, onAgentCharts }) {
  ...
  const liveQ = useQuery({
    queryKey: ["charts", win],
    queryFn: () => window.liberApi.charts(win),
  ...
  const data = (
    (useLive ? live.rows : seedChartsAllowed ? (window.CHARTS || {})[win] : []) || []
  ).map((r) => ({ ...r, surge: surgeMap[r.id] ?? 0, echoes: r.echoes ?? 0 }));
  const ranked = rankBy(data, metric);
  const readTop3 = rankBy(data, "reads")
    .slice(0, 3)
    .map((r) => r.id);
  const max = Math.max(...ranked.map((r) => r[metric]), 1);
  const metricLabel = CH_METRICS.find((m) => m[0] === metric)[1];
  const hot =
    (useLive && live.hotToday) || (seedChartsAllowed ? (window.CHARTS || {}).hotToday : null);
  const sentences = [
    ...((useLive && live.sentences) || (seedChartsAllowed ? window.HOT_SENTENCES : []) || []),
  ].sort((a, b) => b.liners - a.liners);
      ...
      <div className="charts">
        <div className="charts-wrap">
          <div className="crumb" style={{ paddingTop: 24 }}>
            ...
            <span style={{ color: "var(--ink)" }}>榜单</span>
          </div>
          <div className="ch-head">
            ...
            {hot && (
              <div className="ch-hot">
                ...
                <div className="ch-hot-m">
                  {hot.book} · {hot.chap} · {hot.n}
                </div>
              </div>
            )}
          </div>
            ...
            <>
              ...
              <div className="ch-list">
                <div className="ch-row ch-head-row">
                  <span className="ch-rk">#</span>
                  <span className="ch-bk">书</span>
                  ...
                  return (
                    <div className="ch-row" key={r.id} onClick={() => onOpenBook(r.id)}>
                      ...
                      <div className="ch-bk">
                        ...
                        <div>
                          <div className="ch-t">
                            {b.t} {darkhorse && <span className="ch-horse">黑马</span>}
                          </div>
                          <div className="ch-a">{b.a}</div>
                        </div>
                      </div>
                      {metric === "surge" ? (
                        <div className="ch-bar-col">
                          <div className="ch-bar">
                            <div
                              className="ch-fill"
                              style={{
                                width: Math.min(100, r.surge < 0 ? 4 : r.surge / 2) + "%",
                                background: r.surge >= 0 ? "#2e9e6b" : "var(--accent)",
                              }}
                            />
                          </div>
                        ...
                        <div className="ch-bar-col">
                          ...
                          <div className="ch-val">{r[metric].toLocaleString()}</div>
                        </div>
                      )}
                      <span className="ch-cols">
                        {r.reads.toLocaleString()} · {r.lines.toLocaleString()} · {r.convos}
                      </span>
                      <span className="ch-dl">
                        <Delta d={r.delta} />
                      </span>
                    </div>
                  );
              ...
              <div className="ch-foot">
                {metric === "surge"
                  ? "飙升榜按环比增速排序——黑马是绝对量还不高、但涨得最快的书。"
                  : "榜单每 10 分钟从链上重算。"}{" "}
                经 <code>liber.get_charts(window, metric)</code> 对 Agent 开放 · 无人工干预
              </div>
            </>
          ) : (
            <>
                ...
                {sentences.map((s, i) => (
                  <div className="ch-sent" key={s.sid} onClick={() => onOpenBook(s.bookId)}>
                    ...
                    <div className="ch-sent-body">
                      ...
                      <div className="ch-sent-m">
                        <span>
                          {s.book} · {s.chap}
                        </span>
                        <span>{s.liners.toLocaleString()} 人划线</span>
                        <span>{s.convos} 段对话</span>
                        <Surge p={s.surge} />
                      </div>
                    </div>
                    <span className="ch-dl">
              ...
              <div className="ch-foot">
                单句维度的榜单——划线与对话把整本书里最被记住的句子顶了上来。经{" "}
                <code>liber.get_charts("today","sentences")</code> 开放。
              </div>
            </>
          )}
        </div>
          ...
          return (
            <div className="cb-row" key={r.id} onClick={() => onOpenBook(r.id)}>
              ...
              <div className="cb-mid">
                <div className="cb-t">{b.t}</div>
                <div className="cb-m">{r.reads.toLocaleString()} 在读</div>
              </div>
              <Delta d={r.delta} />
            </div>
          );
