...
const SUBJECTS = [
  {
    ...
    match:
      /哲学|思想|道家|儒家|法家|佛|佛典|宗教|神学|道藏|伦理|修辞|oratory|philos|filos|filozof|ethic|relig|theolog|mystic|mistica|mística|confessiones|tao|dao|buddh|esperanto.*gramatiko/,
  },
  {
    id: "politics",
    name: "经济政治",
    ...
    match:
      /经济|政治|政论|兵法|策略|法律|国家|社会|权利|女权|传记|回忆|历史|演说|econom|politic|state|government|rights|social|memoir|biograf|biography|history|historia|istor|kasaysayan|talambuhay|eml[eé]kirat|memorii|m[eé]moires|correspondence|levelek/,
  },
  {
    id: "science",
    name: "科学博物",
    ...
    match:
      /科学|博物|自然|数学|百科|天文|物理|地理|旅行|游记|笔记|文论|家训|语言|语法|字典|读本|science|natural|mathemat|astronom|physics|zoolog|botan|geograph|travel|voyage|journey|adventure|avontuur|aventure|resa|matkakertomus|encyclop|grammar|dictionary|reader|lingvo|učebnice|čítanka|nauka|folkl[oó]r|folklore|volkskunde/,
  },
  {
    id: "literature",
    name: "文学诗",
...
function subjectFor(book) {
  const haystack = foldText(
    [book?.cat, book?.category, book?.sub, book?.t, book?.title].filter(Boolean).join(" "),
  );
...
function subjectOptions(books) {
  ...
  return SUBJECTS.map((subject) => ({ ...subject, count: map.get(subject.id) || 0 })).filter(
...
function Library({ onOpenBook, onOpenCharts }) {
  ...
  useEffLib(() => {
    const off = subscribeCatalog((next) => {
      setBooks(next);
      setTotal(getCatalogTotal());
    });
    loadCatalogBooks()
      .then((next) => {
        setBooks(next);
  ...
  const featureLicense = licenseLabel(feature?.license)
  ...
  const subjectScopedBooks =
    subject === "all" ? scopedBooks : scopedBooks.filter((b) => subjectFor(b) === subject);
  ...
  const list = sortBooks(
    subjectScopedBooks.filter((b) => direction === "all" || directionFor(b) === direction),
    sort,
  );
  const languageSections = langs.map((row) => {
    ...
    const subjectRows = SUBJECTS.map((meta) => ({
      ...meta,
      count: row.subjects.get(meta.id) || 0,
      books: sortBooks(
        sectionBooks.filter((book) => subjectFor(book) === meta.id),
        sort,
      ).slice(0, 4),
    })).filter((item) => item.count > 0);
    return { ...row, meta: languageLabel(row.code), books: sorted.slice(0, 5), subjectRows };
  });
  const selectedLangLabel = currentLang
    ? languageLabel(currentLang.code)
    : { name: "全部语言", sub: "All languages" };
  ...
  const displayCount = currentLang ? `当前显示 ${list.length}` : `按语言和主题子类分区展示`;
  const selectedPath = [
    "全部语言",
      ...
      <div className="app-screen">
        <div className="lib">
          <div className="lib-wrap" style={{ paddingTop: 80 }}>
            ...
            <p className="muted">
              通过 Liber CLI 发布一本 CC0 或 Public Domain EPUB 后，这里会自动显示真实目录。
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="feature-grid">
              ...
              <div className="feature-meta">
                ...
                <div className="feature-stats">
                  <div>
                    <div className="n">{feature.reads}</div>
                    <div className="l">在读</div>
                  </div>
                  <div>
                    <div className="n">{feature.lines}</div>
                    <div className="l">划线</div>
                  </div>
                  <div>
                    <div className="n">{(feature.annos || 0).toLocaleString()}</div>
                    <div className="l">批注</div>
                  </div>
                  <div>
                    <div className="n">{featureLicense}</div>
                    <div className="l">版权</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="lib-wrap">
          ...
          <div className="lib-taxonomy">
            ...
            <div className="tax-metrics">
              ...
              <div>
                <b>{allDirections.length}</b>
                <span>细分类</span>
              </div>
              <div>
                <b>{books.length.toLocaleString("zh-CN")}</b>
                <span>馆藏</span>
              </div>
            </div>
            <div className="lang-grid">
              ...
              {langs.map((row) => {
                ...
                const preview = SUBJECTS.map((item) => ({
                  name: item.name,
                  count: row.subjects.get(item.id) || 0,
                }))
                  .filter((item) => item.count > 0)
          ...
          <div className="lib-bar">
            <span className="title">
              {currentLang ? `${selectedLangLabel.name} 书库` : "全部书库"}
            </span>
            <span className="count">
              {total.toLocaleString("zh-CN")} 卷已入库 · {displayCount}
            </span>
                    ...
                    {section.books.map((b) => (
                      ...
                      >
                        ...
                        <div className="meta">
                          ...
                          <div className="book-taxonomy">
                            <span>{subjectMeta(subjectFor(b)).name}</span>
                            <span>{directionFor(b)}</span>
                          </div>
                        </div>
                      </div>
                    ))}
            ...
            <>
              <div className="subject-panel">
                <div className="subject-panel-head">
                  <span>主题子类</span>
                  <b>
                    {selectedLangLabel.name} · {subjects.length} 类
                  </b>
                </div>
                <div className="subject-grid">
                  ...
                  >
                    ...
                    <span className="sc-sample">
                      覆盖 {subjects.map((row) => row.name).join(" / ")}
                    </span>
                  </button>
                  {subjects.map((row) => {
                    const samples = booksForSubject(scopedBooks, row.id)
                      .slice(0, 3)
                      .map((b) => b.t)
                      .join(" / ");
                    return (
                ...
                {list.map((b) => (
                  <div className="bk-card" key={b.id} {...clickable(() => onOpenBook(b.id))}>
                    ...
                    <div className="meta">
                      ...
                      <div className="a">{b.a}</div>
                      <div className="book-taxonomy">
                        <span>{languageLabel(languageCodeFor(b)).name}</span>
                        <span>{subjectMeta(subjectFor(b)).name}</span>
                        <span>{directionFor(b)}</span>
                      </div>
                      <div className="stat">
