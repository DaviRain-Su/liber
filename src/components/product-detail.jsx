...
function Detail({ bookId, onOpenReader, onOpenCert, onBack, onOpenAgents }) {
  ...
  const bookQ = useQuery({
    queryKey: ["book", bookId, "detail"],
    queryFn: () => window.liberApi.books.get(bookId),
    enabled: !!window.liberApi,
  });
  const detail =
    bookQ.data && bookQ.data.book
      ? {
          book: bookQ.data.book,
          toc: bookQ.data.toc,
          highlights: bookQ.data.highlights,
          reviews: bookQ.data.reviews,
        }
      : {
          book: seedBook,
          ...
          <div className="detail-grid">
            ...
            <div className="detail-rail">
              ...
              <div className="detail-mini">
                <div>
                  <div className="n">{book.pages}</div>
                  <div className="l">章 / 节</div>
                </div>
                <div>
                  <div className="n">{String(book.words || "").replace("约 ", "")}</div>
                  <div className="l">篇幅</div>
                </div>
                <div>
                  <div className="n">{book.reads}</div>
                  <div className="l">在读</div>
                </div>
                <div>
                  <div className="n">{book.lines}</div>
            ...
            <div className="detail-main">
              ...
              <div className="agent-track" {...clickable(onOpenAgents)}>
                <div className="at-avas">
                  {["友", "导", "思", "考"].map((s, i) => (
                    <span
                      ...
                      className="at-ava"
                      style={{
                        background: ["var(--accent)", "#2e7d57", "#3a4fb0", "#9a5b2e"][i],
                        marginLeft: i ? -9 : 0,
                        zIndex: 9 - i,
                      }}
                    >
                ...
                <div className="at-txt">
                  <b>4 个 Agent</b> 正在追踪这本书的讨论 · 导读 Agent 已生成一份摘要
                </div>
              ...
              <div className="dsec">
                ...
                <div className="proof-card">
                  <div className="ph">
                    {book.t} · 链上记录 <span className="ok">● 已永久存证</span>
                  </div>
                  <div className="pr">
                    <span className="k">blob_id</span>
                    <span className="v">{book.blob}</span>
                  </div>
                  <div className="pr">
                    <span className="k">backup</span>
                    <span className="v">{book.backup}</span>
                  </div>
                  <div className="pr">
                  ...
                  <div className="pr">
                    ...
                    <span className="v">{book.license || "CC0-1.0"}</span>
                  </div>
                </div>
              </div>
