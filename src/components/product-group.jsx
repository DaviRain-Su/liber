...
function GroupsList({ onOpenGroup, onBack }) {
  ...
  const groups = Array.isArray(groupsQ.data?.groups)
    ? groupsQ.data.groups
    : allowSeedFallback
      ? window.GROUPS || []
      : [];
      ...
      <div className="grp-screen">
        <div className="grp-wrap">
          <div className="crumb" style={{ paddingTop: 24 }}>
            <a onClick={onBack}>共读</a> <span>/</span>{" "}
            <span style={{ color: "var(--ink)" }}>共读小组</span>
          </div>
          ...
          <div className="gl-grid">
            {groups.length ? (
              ...
              <div className="pf-empty">
                还没有真实共读小组。真实馆藏会生成可加入的小组，加入后成员与讨论会写入数据库。
              </div>
            )}
          </div>
        </div>
...
function Group({ groupId, onBack, onOpenReader }) {
  ...
  const allowSeedFallback = !window.liberApi && !hasLiveCatalog;
  const fallback =
    (allowSeedFallback ? window.GROUPS || [] : []).find((x) => x.id === groupId) ||
    (allowSeedFallback ? window.GROUPS?.[0] : null);
  ...
  const add = () => {
    ...
    setReplies((r) => [
      { u: "你", color: "#3a4fb0", when: "刚刚", chap, t: text, up: 0, replies: 0, mine: true },
      ...r,
    ]);
  ...
  const toggleJoin = () => {
    ...
    if (window.liberApi)
      window.liberApi.groups
        .join(g.id)
        .then((r) => {
          if (typeof r?.joined === "boolean") setJoined(r.joined);
        })
        .catch(() => {});
  };
  ...
  return (
    <div className="app-screen">
      <div className="grp-screen">
        {/* banner */}
        <div
          className="grp-banner"
          style={{
            background: `linear-gradient(120deg, ${g.color}, color-mix(in oklab, ${g.color} 55%, #14110c))`,
          }}
        >
        ...
        <div className="grp-wrap">
          ...
          <div className="grp-bar">
            <div className="gbar-avas">
              ...
              <span className="gbar-mc">
                {g.members} 人在读 · 组长 {g.lead}
              </span>
            </div>
          ...
          <div className="grp-grid">
            ...
            <main className="grp-main">
              <div className="grp-progress">
                <div className="gp-h">
                  <span>本周进度 · {g.weekRange}</span>
                  <span className="gp-pct">{g.progressPct}%</span>
                </div>
                <div className="gp-track">
                  <div
                    className="gp-fill"
                    style={{ width: g.progressPct + "%", background: g.color }}
                  />
                </div>
                <div className="gp-sub">
                  {g.members} 人中约 {Math.round((g.members * g.progressPct) / 100)}{" "}
                  人已读到本周进度 · 组内已有 {g.annos} 条批注
                </div>
              </div>
                ...
                <div className="grp-topanno">
                  ...
                  <div className="ta-row">
                    ...
                    <div>
                      ...
                      <div className="tx">{g.topAnno.t}</div>
                    </div>
                    <span className="up">
                      {I.up} {g.topAnno.up}
                    </span>
              ...
              <div className="grp-disc">
                ...
                <div className="gd-compose">
                  <span className="ava" style={{ background: "#3a4fb0" }}>
                    你
                  </span>
                  <input
                    placeholder={`聊聊本周的 ${g.weekRange}…`}
                    value={draft}
                    ...
                    <div className="gd-post" key={i}>
                      ...
                      <div className="gd-body">
                        <div className="gd-top">
                          ...
                          <span className="chap">{d.chap}</span>
            ...
            <aside className="grp-aside">
                ...
                <div className="sched">
                  {(g.schedule || []).map((s, i) => (
                    <div className={`sch-row ${s.state}`} key={i}>
                      ...
                      <div className="sch-mid">
                        <div className="sch-wk">{s.wk}</div>
                        <div className="sch-chap">{s.chap}</div>
                      </div>
              ...
              <div className="sh-card">
                ...
                <div className="mem-grid">
                  ...
                  {g.members > (g.memberAvatars || []).length && (
                    <div className="mem more">+{g.members - (g.memberAvatars || []).length}</div>
                  )}
                </div>
              </div>
            </aside>
