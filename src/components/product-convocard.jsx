...
const CC_ICO = {
  fork: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="6" cy="5" r="2.4"/><circle cx="18" cy="5" r="2.4"/><circle cx="12" cy="19" r="2.4"/><path d="M6 7.4v3c0 2 1.6 3 3.5 3.4M18 7.4v3c0 2-1.6 3-3.5 3.4M12 14v2.6"/></svg>,
  up: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m6 14 6-6 6 6"/></svg>,
  chat: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M21 12a8 8 0 0 1-11.5 7.2L4 20l1-4.2A8 8 0 1 1 21 12Z"/></svg>,
  save: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M6 4h12v16l-6-4-6 4z"/></svg>,
  saved: <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1.8"><path d="M6 4h12v16l-6-4-6 4z"/></svg>,
  img: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8.5" cy="9.5" r="1.6"/><path d="m4 18 5-5 4 4 3-3 4 4"/></svg>,
    ...
    <div className="cf cform-card">
      <div className="cf-head">
        <div className="cf-anchor">
          ...
          <div className="meta"><div className="bk">{convo.bookT}</div><div className="ch">{convo.chap}</div></div>
        </div>
...
function InsightCardForm({ convo, expanded, onToggleExpand, onFork, onSave, saved, onOpenTree }){
  ...
  return (
    <div className="cf cform-insight">
      ...
      <div className="ci-from">由一段关于《{convo.bookT} · {convo.chap}》的对话提炼<br/>原文：<b>「{convo.quote.slice(0,12)}…」</b></div>
      {!expanded
        ? <div className="ci-expand" onClick={onToggleExpand}>看背后的完整对话 · {convo.msgs.length} 条 ↓</div>
  ...
  return (
    <div className="ft-node">
      <div className="ft-card">
        <span className="ava" style={{ background: node.color }}>{node.ava}</span>
...
function ForkTreeModal({ convo, onClose, onFork }){
  ...
  const total = (function count(nodes){ return nodes.reduce((s,n)=> s+1+count(n.children||[]), 0); })(tree);
  return (
    <>
      ...
      <div className="forktree-modal">
        <div className="ft-head">
          <div>
            <div className="ft-kick">{CC_ICO.fork} 对话树 · 这段被接力了 {total} 次</div>
            <div className="ft-sub">{convo.bookT} · {convo.chap}</div>
          </div>
          <span className="x" onClick={onClose}>{CC_ICO.x}</span>
        </div>
        <div className="ft-rootcard">
          <div className="ft-root-q">「{convo.title || convo.insight || convo.msgs[0].t}」</div>
          <div className="ft-root-by"><span className="ava" style={{background:convo.author.color}}>{convo.author.ava}</span>原对话 · {convo.author.name} · {convo.msgs.length} 条</div>
        </div>
...
async function exportInsightImage(convo, theme = "ink"){
  ...
  let yy = 430;
  lines.forEach(ln => { x.fillText(ln, pad, yy); yy += lineH; });
  ...
  yy += 44;
  const orig = "原文：「" + (convo.quote.length>20 ? convo.quote.slice(0,20)+"…" : convo.quote) + "」";
...
function wrapCJK(ctx, text, maxW){
  ...
  for (const chTok of Array.from(text)){
    ...
    if (ctx.measureText(test).width > maxW && line){ lines.push(line); line = chTok; }
    else line = test;
  }
