...
function gatherHighlights(serverReading = {}, summary = null) {
  ...
  if (!hasLive)
    (window.SEED_HL || []).forEach((h) => {
      if (catalogTitles.has(h.book)) out.push({ ...h, seed: true });
    });
  getCatalogBooks().forEach((b) => {
    let hl = {},
      nt = {};
    try {
      hl = JSON.parse(localStorage.getItem("liber.hl." + b.id)) || {};
    } catch {}
    try {
      nt = JSON.parse(localStorage.getItem("liber.nt." + b.id)) || {};
    } catch {}
    ...
    if (seedContent)
      (seedContent.chapters || []).forEach((c) =>
        c.paras.flat().forEach((s) => {
          sentMap[s.id] = { t: s.t, chap: "第" + c.n + "章" };
        }),
      );
...
function Notebook({ onOpenBook }) {
  ...
  const [works, setWorks] = useSn(() => {
    try {
      return JSON.parse(localStorage.getItem("liber.works")) || [];
    } catch {
      return [];
    }
  });
  useEffN(() => {
    ...
    window.liberApi.works
      .list()
      .then((r) => {
        if (r && Array.isArray(r.works) && r.works.length)
          setWorks(r.works.map((w) => ({ ...w, when: w.when || "已发布" })));
      })
      .catch(() => {});
  }, []);
  useEffN(() => {
    ...
    Promise.all(
      ...
      if (!live) return;
      const next = {};
      rows.filter(Boolean).forEach(([id, r]) => {
        next[id] = r;
      });
        ...
        <div className="nb-wrap">
          <div className="nb-head">
            <div>
              ...
              <p className="nb-sub">
                {all.length} 条划线 · {all.filter((h) => h.note).length} 条批注 · {summaries.length}{" "}
                篇 AI 摘要 · 全部永久存证
              </p>
            </div>
                  ...
                  summaries.map((s, i) => (
                    <div className="nb-summary" key={i}>
                      <div className="ns-head">
                        <span className="orb" />
                        <div>
                          <div className="ns-chap">{s.chap}</div>
                    ...
                    <div className="nb-hl" key={i}>
                      ...
                      <div className="hl-c">
                        ...
                        <div className="hl-meta">
                          ...
                          <span>{h.when}</span>
                          {h.live && <span className="live">你刚标的</span>}
                        </div>
                      </div>
                    </div>
                  ))
                  ...
                  works.map((w, i) => (
                    <div className="nb-work" key={i}>
                      <div className="nw-top">
                        <div className="nw-cc">CC0</div>
                        <div className="nw-when">{w.when}</div>
                      </div>
                      <div className="nw-title">{w.title}</div>
                      <div className="nw-body">
                        {w.body.length > 180 ? w.body.slice(0, 180) + "…" : w.body}
                      </div>
                      ...
                      {w.id && cmtFor === w.id && (
                        <CommentsPanel targetType="work" targetId={w.id} />
                      )}
                    </div>
                  ))
                ) : (
      ...
      {writeOpen && (
        <WriteComposer
          ...
          onPublish={(w) => {
            const next = [w, ...works];
            setWorks(next);
            localStorage.setItem("liber.works", JSON.stringify(next));
            if (window.liberApi) window.liberApi.works.publish(w.title, w.body).catch(() => {});
            setWriteOpen(false);
            setTypeF("work");
          }}
        />
