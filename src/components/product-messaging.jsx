...
function QuoteBlock({ quote }) {
  return (
    <div className="dm-quote">
      ...
      <div className="dm-quote-src">
        {quote.book}
        {quote.chap ? ` · ${quote.chap}` : ""}
        {quote.note ? " · 批注" : ""}
      </div>
    </div>
  );
...
function Messenger({ startWith, onClose }) {
  ...
  const loadThreads = () => {
    api?.messages
      ?.threads()
      .then((r) => {
        if (Array.isArray(r?.threads)) setThreads(r.threads);
      })
      .catch(() => {});
  };
  useMsgE(() => {
  ...
  const list = (() => {
    ...
    if (startWith?.userId && !arr.some((t) => t.userId === startWith.userId)) {
      arr.unshift({
        userId: startWith.userId,
        name: startWith.name || "读者",
        handle: startWith.handle || "",
        color: startWith.color || "#3a4fb0",
        seal: String(startWith.name || "读")[0],
        lastText: "",
        lastFromMe: false,
        unread: 0,
      });
  ...
  const openPerson = () =>
    partner &&
    window.openProfile &&
    window.openProfile({ userId: partner.userId, name: partner.name });
  const send = () => {
    const t = draft.trim();
    if (!t || !active) return;
    setMsgs((m) => [
      ...m,
      { id: "local" + Date.now(), fromMe: true, t, quote: null, when: "刚刚" },
    ]);
    setDraft("");
    api?.messages
      ?.send(active, t)
            ...
            {list.map((p) => (
              ...
              >
                ...
                <div className="dm-convo-main">
                  <div className="dm-convo-nm">
                    {p.name}
                    {p.unread > 0 && <span className="dm-unread-dot" />}
                  </div>
                  <div className="dm-convo-last">
                    {p.lastText ? (p.lastFromMe ? "你：" : "") + p.lastText : "开始一段对话…"}
                  </div>
                </div>
              </div>
            ))}
            {list.length === 0 && (
              <div className="dm-none" style={{ margin: "24px 14px", fontSize: 14 }}>
                还没有私信。
                <br />
                在书友主页点「私信」开始。
              </div>
            ...
            <>
              ...
              <div className="dm-body" ref={bodyRef}>
                ...
                {!loading && msgs.length === 0 && (
                  <div className="dm-empty">给 {partner.name} 写第一条消息吧。</div>
                )}
                {msgs.map((m, i) => (
              ...
              <div className="dm-compose">
                <input
                  placeholder={`给 ${partner.name} 发消息…`}
                  value={draft}
...
function Notifications({ onClose, onOpenBook }) {
  ...
  const load = () =>
    api?.notifications
      ?.list()
      .then((r) => {
        if (Array.isArray(r?.notifications)) setItems(r.notifications);
      })
      .catch(() => {});
  useMsgE(() => {
    load();
  ...
  const act = (n) => {
    markOne(n);
    if (n.kind === "follow" && n.actorId) {
      onClose();
      window.openProfile && window.openProfile({ userId: n.actorId, name: n.who });
      return;
    }
    if (n.kind === "dm" && n.actorId) {
      onClose();
      window.dispatchEvent(
        new CustomEvent("liber-open-dm", { detail: { userId: n.actorId, name: n.who } }),
      );
      return;
    }
    if (n.book) {
      onClose();
      onOpenBook && onOpenBook(n.book);
      return;
    }
  };
  return (
            ...
            >
              ...
              <div className="notif-body">
                ...
                <div className="notif-meta">
                  <span className="notif-kind">{ICON[n.kind] || I.bell}</span>
                  <span>{n.when}</span>
                </div>
              </div>
            </div>
