...
function SendFlow({ presetToken, onClose }){
  ...
  return (<Sheet title={titles[phase]} step={phase!=="done"?`${idx+1} / 5`:null} onBack={back} onClose={onClose}>
    ...
    {phase==="pk" && <PasskeyGate label="用通行密钥确认转账" sub={`${amount} ${token.sym} → ${recipient&&recipient.name}`} onDone={()=>setPhase("done")} onCancel={()=>setPhase("review")}/>}
    {phase==="done" && (<div className="done-state"><div className="done-mark">{WI.check}</div><div className="dt">转账已提交</div><div className="dsub">{amount} {token.sym} → {recipient&&recipient.name}<br/>已在 {token.chain} 上广播，等待确认。</div><div className="done-hash">{WI.ext} {hash.current}</div></div>)}
