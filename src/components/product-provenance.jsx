...
function provFacts(n){
  ...
  return {
    ...
    by: ag ? ag.by : null,
    sig: ag ? ag.sig : (n.sig || "sui:0x" + (Math.abs(hashStr(n.u||n.name||"x")).toString(16)+"0000").slice(0,4) + "…" + (Math.abs(hashStr(n.t||"")).toString(16)+"00").slice(0,2) + "f1"),
...
function ProvBadge({ note, mine }){
  ...
  const onClick = (e) => { const r = e.currentTarget.getBoundingClientRect(); setOpen({ x: r.left, y: r.bottom + 6 }); };
  return (
    <>
      <span className="prov" onClick={onClick}>
        <span className="prov-nm">{f.name}{mine && " · 你"}</span>
        {f.isAgent && <span className="prov-kind">AI Agent</span>}
        {f.signed && <span className="prov-sig" title="链上签名">✓ 已签名</span>}
...
function ProvDetail({ f, pos, onClose }){
  return (
    <>
      <div className="drawer-scrim" style={{ background:"transparent", zIndex:852 }} onClick={onClose}/>
      <div className="prov-detail" style={{ left: Math.min(pos.x, window.innerWidth-300), top: pos.y }}>
        <div className="pd-head">
          <span className={"pd-orb"+(f.isAgent?" agent":"")}>{f.isAgent ? I.agent : f.name[0]}</span>
          <div>
        ...
        <div className="pd-rows">
          {f.by && <div className="pd-row"><span className="k">来源</span><span className="v">{f.by}</span></div>}
          <div className="pd-row"><span className="k">签名</span><span className="v mono">{f.sig}</span></div>
          <div className="pd-row"><span className="k">状态</span><span className="v ok">● 已链上签名 · 任何人可验证</span></div>
          <div className="pd-row"><span className="k">被引用</span><span className="v">{f.cited.toLocaleString()} 次{f.isAgent && " · 可被其他 Agent 引用"}</span></div>
